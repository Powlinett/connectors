...
from models.interfaces.common import FrozenBaseModel
...
class _ConfigLoaderConnectorTemplate(ABC, FrozenBaseModel):
    """Interface for loading ConnectorTemplate dedicated configuration."""

    api_base_url: HttpUrl = Field(
        ...,
        description="ConnectorTemplate API base URL.",
    )
    api_token: SecretStr = Field(
        ...,
        description="ConnectorTemplate API token.",
    )
    ...
    def __init__(self) -> None:
        """Initialize ConnectorTemplate dedicated configuration."""
        try:
            FrozenBaseModel.__init__(
                self,
            ...
            error_message = "Invalid ConnectorTemplate configuration."
            logger.error(error_message)
            raise ConfigRetrievalError(error_message) from exc
...
class ConfigLoader(ABC, FrozenBaseModel):
    ...
    opencti: _ConfigLoaderOCTI = Field(
        ...,
        description="OpenCTI config.",
    )
    connector: _ConfigLoaderConnector = Field(
        ...,
        description="Connector config.",
    )
    connector_template: _ConfigLoaderConnectorTemplate = Field(
        ...,
        description="ConnectorTemplate config.",
    )
    ...
    def __init__(self) -> None:
        ...
        try:
            FrozenBaseModel.__init__(
                ...
                connector_template=self._connector_template,
            )
    ...
    @property
    @abstractmethod
    def _connector_template(self) -> _ConfigLoaderConnectorTemplate:
        pass
    ...
    def to_dict(self, token_as_plaintext: bool = False) -> dict[str, Any]:
        ...
        return {
            ...
            "connector_template": {
                "api_base_url": self.connector_template.api_base_url,
                "api_token": (
                    self.connector_template.api_token.get_secret_value()
                    if token_as_plaintext
                    else self.connector_template.api_token
                ),
                "import_start_date": self.connector_template.import_start_date,
                "tlp_level": self.connector_template.tlp_level,
            },
        }
